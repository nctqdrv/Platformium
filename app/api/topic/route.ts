...
export async function POST(req: Request) {
  try {
    ...
    console.log('Topic analizi için gelen içerik:', content);

    const anthropic = new Anthropic({
      apiKey: process.env.ANTHROPIC_API_KEY || '',
    });

    const systemPrompt = `Verilən mətnləri diqqətlə təhlil et. Bu şərhdə təsvir edilən mövzunu (topic) dəqiq müəyyən etmək lazımdır. Xüsusilə aşağıdakı kateqoriyalara diqqət yetir:
...
Şərhin mövzusunu (topic-ini) maksimum 2-3 sözdən ibarət qısa formada təyin et. Cavabını yalnız mövzu (topic) sözlərindən ibarət et, əlavə izahat vermə.
...
Əgər yazılan mövzu yuxarıdakılardan heç biri ilə əlaqəli deyilsə, o zaman Əlaqəsiz cavabını ver.
